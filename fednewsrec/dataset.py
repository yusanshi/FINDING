...
from fednewsrec.shared import args, logger
from fednewsrec.utils import load_from_cache
    ...
    def _process_news(news_path):
        ...
        for attribute in args.dataset_attributes['news']:
            if attribute in ['category', 'subcategory']:
                ...
                attribute_length = len(news[attribute][0])
                news_elements.append(np.array(news[attribute].tolist()))
            else:
                raise ValueError

            news_pattern[attribute] = (current_length,
                                       current_length + attribute_length)
            current_length += attribute_length
...
class UserDataset(Dataset):
    ...
    instead it means a user with unique `args.dataset_attributes['behaviors']`.
    In this way, a regular user with two different history are
    viewed as two users.
    """
