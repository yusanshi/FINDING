...
UServerT2V1::UServerT2V1(string u_serverIP, int u_serverPort, string t_serverIP,
                         int t_serverPort, unsigned k, int max_round,
                         int variance_bound) {
  this->k = k;
  ...
  while (r < this->max_round) {
    ...
    for (auto &iter : this->A_r) {
      ...
      for (int i = 0; i < this->k; i++) {
        ...
        if (0 >
            send(this->t_serverSocket, &cluster_index, sizeof(uint32_t), 0)) {
        ...
        distance = FHE_HM(this->cipherpoints[iter.first],
                          this->centroids[this->rev_centroids_clusters[i]]);
        this->sendStream(this->distanceToStream(distance),
                         this->t_serverSocket);
      ...
      for (unsigned t = 0; t < this->k; t++) {
        ...
        this->receiveStream(this->t_serverSocket,
                            to_string(t) + "-clusterindex.dat");
    ...
    if (r < this->max_round) {
      ...
      for (unsigned i = 0; i < this->k; i++) {
        ...
        if (0 >
            send(this->clientSocket, &centroid_index, sizeof(uint32_t), 0)) {
        ...
        this->sendStream(this->clusterSizeToStream(this->clusters_size[i]),
                         this->clientSocket);
        ...
        this->sendStream(this->centroidToStream(total_of_coef),
                         this->clientSocket);
...
void UServerT2V1::socketCreate() {
  ...
  if (this->u_serverSocket < 0) {
    ...
    int opt = 1;
    setsockopt(this->u_serverSocket, SOL_SOCKET, SO_REUSEADDR, &opt,
               sizeof(opt));
...
void UServerT2V1::socketBind() {
  ...
  if (bind(this->u_serverSocket, (sockaddr *)&u_serverAddress,
           sizeof(u_serverAddress)) < 0) {
    perror("BIND ERROR");
    exit(1);
  } else {
    string message = "Socket bound successfully to :[" + u_serverIP + ":" +
                     to_string(this->u_serverPort) + "]";
    print(message);
  }
}
...
void UServerT2V1::swapA() { this->A = this->A_r; }
...
void UServerT2V1::resultsToKClient() {
  ...
  for (auto &iter : this->A) {
    ...
    for (unsigned i = 0; i < this->k; i++) {
      this->sendStream(this->resultToStream(iter.second[i]),
                       this->clientSocket);
