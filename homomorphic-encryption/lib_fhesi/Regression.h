...
bool LoadData(Matrix<ZZ> &rawData, vector<ZZ> &labels, unsigned &dim,
...
double BatchData(vector<vector<Plaintext>> &ptxtData,
                 vector<Plaintext> &ptxtLabels, const Matrix<ZZ> &rawData,
                 const vector<ZZ> &labels, const FHEcontext &context) {
  double start(clock());
  ...
  for (unsigned i = 0; i < rawData.NumRows(); i += batchSize) {
    ...
    for (unsigned j = 0; j < rawData.NumCols(); j++) {
      ...
      for (unsigned k = i; ((k < i + batchSize) && (k < rawData.NumRows()));
           k++) {
...
class Regression {
 public:
  ...
  void AddData(const vector<vector<Plaintext>> &ptxtData,
               const vector<Plaintext> &ptxtLabels) {
  ...
  void Regress(vector<Ciphertext> &theta, Ciphertext &det) const {
    ...
    dataCopy *= last;
    dataCopy.MapAll(
        [this](Ciphertext &ctxt) { this->keySwitch.ApplyKeySwitch(ctxt); });
